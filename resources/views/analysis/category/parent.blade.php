                                ...
                                <div class="card-body px-5">
                                    <div class="row mt-3 mb-4" style="min-height: 250px">
                                        <div class="col position-relative">
                                            <div class="z-0 position-absolute top-50 start-50 translate-middle">
                                                <h3 class="h4 text-center fw-bold text-secondary" style="width: 100px;">
                                                    {{ $parentCategory['name'] }}
                                                </h3>
                                                <h4 class="fw-bold text-center" style="color: #FE6D73;">
                                                    ¥{{ number_format(floor($parentCategory['total_amount'])) }}
                                                </h4>  
                                            </div>
                                            <div class="z-1 position-absolute top-50 start-50 translate-middle">
                                                ...
                                                @foreach ($parentCategory['childCategories'] as $childCategory)
                                                    <tr class="align-middle">
                                                        <th class="border-0 pb-0">
                                                            <a href="{{ route('analysis.category.child', [$parentCategory['id'], $childCategory['id']]) }}" class="btn fw-bold text-decoration-underline text-dark p-0">
                                                                {{ $childCategory['name']}}
                                                            </a>
                                                        </th>
                                                        <td class="text-end border-0 pb-0">
                                                            ¥{{ number_format(floor($childCategory['total_amount'])) }}
                                                        </td>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td class="fw-light text-secondary text-end py-0" style="font-size: x-small;">
                                                            {{ floor($childCategory['percentage_of_total']) }}%
                                                        </td>
                                                    </tr>
                                                @endforeach
                                            </table>
                                    ...
                                    <div class="mb-3">
                                        <h5 class="h4 ps-5">{{ $parentCategory['name'] }}</h5>
                                        <canvas id="childBarChart_{{ $loop->index }}" height="130px"></canvas>
                                        <div class="childBarData"
                                            data-childBarLabels="{{ json_encode($parentCategory['monthly_labels']) }}"
                                            data-childBarColors="{{ json_encode('#' . $parentCategory['color_hex']) }}"
                                            data-childBarAmounts="{{ json_encode($parentCategory['monthly_amounts']) }}">
                                        </div>
                                    </div>
                                </div>
                            </div>
