                        ...
                        <div class="row row-cols-3 bg-light shadow rounded-3 p-4 mb-5">
                            @foreach ($user->parentCategories as $user_parent_category)
                                @if ($user_parent_category->type == 'expense')
                                    <div class="col my-2">
                                        <a href="{{ route('category.child.show', $user_parent_category->id) }}" class="btn btn-lg px-3 text-truncate {{ Request::segment(3) == $user_parent_category->id ? 'active' : '' }}" style="color: #{{ $user_parent_category->color_hex }}">
                                    ...
                                    <div class="d-flex flex-wrap mx-3">
                                        @foreach ($parent_category->childCategories as $parent_child_category)
                                            <div class="mx-4 my-3 pe-2">
                                                <a href="{{ route('category.child.edit', [$parent_child_category->parent_category_id, $parent_child_category->id]) }}" class="h5 link-offset-3 {{ Request::segment(4) == $parent_child_category->id ? 'poppins-bold' : '' }}" style="text-decoration-thickness: {{ Request::segment(4) == $parent_child_category->id ? '3px' : '' }};">{{ $parent_child_category->name }}</a>
                                            </div>
                                        @endforeach
                                    </div>
                                </div>
                        ...
                        <div class="row row-cols-3 bg-light shadow rounded-3 p-4">
                            @foreach ($user->parentCategories as $user_parent_category)
                                @if ($user_parent_category->type == 'income')
                                    <div class="col my-2">
                                        <a href="{{ route('category.parent.edit', $user_parent_category->id) }}" class="h5 link-offset-3 px-3 text-truncate {{ Request::segment(3) == $user_parent_category->id ? 'fw-bold' : '' }}" style="color: #{{ $user_parent_category->color_hex }}; text-decoration-thickness: {{ Request::segment(3) == $user_parent_category->id ? '3px' : '' }};">
                                            <img src="{{ asset('images/category-icon/' . $user_parent_category->icon_path . '.svg') }}" alt="{{ $user_parent_category->icon_path }}" width="25px" class="me-1 filter-{{ $user_parent_category->color_hex }}">
                                            {{ $user_parent_category->name }}
                                        </a>
                                    </div>
                    ...
                    <div class="row gx-5 mb-4">
                        ...
                        <div class="col ps-4">
                            <div class="row row-cols-4 gx-1">
                                ...
                                <div class="form-check p-0">
                                    <input class="btn-check" type="radio" id="edit_#FFE9A7" value="FFE9A7" name="color_hex" {{ $parent_category->color_hex == 'FFE9A7' ? 'checked' : '' }}>
