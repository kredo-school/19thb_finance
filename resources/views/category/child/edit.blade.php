            ...
            <div class="row justify-content-center">
                                ...
                                <div class="row row-cols-auto bg-light shadow rounded-3 p-4 gx-4 gy-2 mb-5">
                                    @foreach ($user->parentCategories as $user_parent_category)
                                        @if ($user_parent_category->type == 'expense')
                                            <div class="col p-0">
                                                <a href="{{ route('category.child.show', $user_parent_category->id) }}" class="btn btn-lg px-3 {{ Request::segment(2) == $user_parent_category->id ? 'active' : '' }}" style="color: #{{ $user_parent_category->color_hex }}" title="{{ $user_parent_category->name }}">
                ...
                <div class="col-lg-6 p-4">
                    ...
                    <div class="card border-0 shadow mt-lg-5 my-3 p-5">
                        <form action="{{ route('category.child.update', [$parent_category->id, $child_category->id]) }}" method="post">
                            @csrf
                            @method('PATCH')
                            ...
                            <div class="row justify-content-center">
                                ...
                                <div class="col-auto">
                                    <input type="text" class="fs-5 text-center border-0 border-bottom border-dark bg-light py-2" name="name" id="name" value="{{ old('name', $child_category->name) }}" placeholder="Child Category">
                                    @error('name')
                            ...
                            <div class="d-flex justify-content-end mb-4">
                                <button type="button" class="btn p-0" title="Delete" data-bs-toggle="modal" data-bs-target="#delete-child_category-{{$child_category->id}}">
                        ...
                        {{-- modal : delete child_category --}}
                        @include('category.child.modal.delete')
                    </div>
                </div>
            </div>
        </div>
